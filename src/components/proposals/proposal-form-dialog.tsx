...
export function ProposalFormDialog({ open, onOpenChange }: Props) {
  ...
  const { register, handleSubmit, control, formState: { errors }, reset } = useForm<FormInput>({
    ...
    defaultValues: { currency: 'AED', subtotal: 0, totalAmount: 0 },
  })

  const mutation = useMutation({
    mutationFn: (data: FormData) => fetch('/api/proposals', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(data),
    }).then(async (r) => {
      if (!r.ok) { const e = await r.json(); throw new Error(e.error || 'Failed to create proposal') }
      return r.json()
    }),
    onSuccess: () => {
        ...
        <form onSubmit={handleSubmit((data) => mutation.mutate(data as FormData))} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2">
              ...
              <Textarea id="executiveSummary" {...register('executiveSummary')} rows={3} className="mt-1" />
            </div>

            <div className="col-span-2">
              <Label htmlFor="scopeOfWork">Scope of Work</Label>
              <Textarea id="scopeOfWork" {...register('scopeOfWork')} rows={3} className="mt-1" />
            </div>

            <div>
              <Label htmlFor="deliverables">Deliverables</Label>
              <Textarea id="deliverables" {...register('deliverables')} rows={3} className="mt-1" />
            </div>

            <div>
              <Label htmlFor="timeline">Timeline</Label>
              <Textarea id="timeline" {...register('timeline')} rows={3} className="mt-1" />
            </div>
          </div>
