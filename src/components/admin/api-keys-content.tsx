...
export function ApiKeysContent() {
  ...
  return (
    <div className="space-y-6 animate-slide-in">
      <PageHeader title="API Keys" description="Programmatic access to your CRM API (acts with your permissions)">
        <div className="flex items-center gap-2">
          <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Key name" className="h-9 w-44" />
          <Button size="sm" disabled={!name.trim() || createMutation.isPending} onClick={() => createMutation.mutate()}>
            {createMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Plus className="h-4 w-4 me-2" />} Create
          </Button>
        </div>
      </PageHeader>
          ...
          <div key={k.id} className="flex items-center justify-between gap-3 p-3.5">
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                <p className="text-sm font-medium truncate">{k.name} {k.revokedAt && <span className="text-[11px] text-red-600">(revoked)</span>}</p>
                <p className="text-xs text-muted-foreground font-mono">{k.keyPrefix}… · {k.lastUsedAt ? `used ${formatRelativeDate(k.lastUsedAt)}` : 'never used'}</p>
              </div>
            </div>
            {!k.revokedAt && (
              <Button variant="ghost" size="icon" className="h-8 w-8" disabled={revokeMutation.isPending} onClick={() => revokeMutation.mutate(k.id)}>
      ...
      <Dialog open={newKey !== null} onOpenChange={(o) => !o && setNewKey(null)}>
        <DialogContent>
          ...
          <div className="space-y-2 py-2">
            <Label>Copy this key now — it won&apos;t be shown again.</Label>
            <div className="flex items-center gap-2">
              <code className="flex-1 truncate rounded-md border bg-muted px-3 py-2 text-xs">{newKey}</code>
              <Button variant="outline" size="icon" onClick={() => { navigator.clipboard?.writeText(newKey ?? ''); toast.success('Copied') }}><Copy className="h-4 w-4" /></Button>
            </div>
            <p className="text-xs text-muted-foreground">Send as <code>Authorization: Bearer {'<key>'}</code> to the CRM API.</p>
          </div>
