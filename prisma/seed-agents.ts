...
const ROLE_MATRIX: Record<string, { name: string; nameAr: string; permissions: string[] }> = {
  ...
  'dept-08-finance': {
    ...
    permissions: g(['invoices', 'payments', 'expenses', 'quotations', 'subscriptions', 'bundles', 'payroll'], ['view', 'create', 'edit', 'export', 'approve']).concat(g(['reports'], ['view', 'export'])).concat(g(['dashboard'], VIEW)),
  },
  'dept-09-legal': {
  ...
  'dept-15-partners': {
    ...
    permissions: g(['companies', 'contacts', 'contracts', 'client_services'], CRU).concat(g(['dashboard', 'reports'], VIEW)),
  },
