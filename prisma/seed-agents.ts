...
const ROLE_MATRIX: Record<string, { name: string; nameAr: string; permissions: string[] }> = {
  ...
  'dept-08-finance': {
    ...
    permissions: g(['invoices', 'payments', 'expenses', 'quotations', 'subscriptions', 'bundles'], ['view', 'create', 'edit', 'export', 'approve']).concat(g(['reports'], ['view', 'export'])).concat(g(['dashboard'], VIEW)),
  },
  'dept-09-legal': {
