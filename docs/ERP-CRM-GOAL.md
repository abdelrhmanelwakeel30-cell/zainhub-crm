...
- [~] **E-3** Finance/Accounting — **backend done**: `Account` (chart of accounts, AccountType enum) + `JournalEntry` + `JournalLine` models + migration; `accounting` perm in both seeds + Finance role; APIs — `/api/accounts` (GET/POST, unique code), `/api/journal-entries` (GET/POST with **double-entry balanced validation** — debits==credits>0, debit XOR credit per line, tenant-owned accounts, entry+lines in a transaction), `/api/journal-entries/[id]` (GET + PATCH post→accounting:approve). Verified E2E (sales 403; finance creates accounts + balanced JE-0001 201 / unbalanced 422). **Next:** accounting UI (chart of accounts + JE entry) + ledgers/trial-balance + GL postings from payroll/invoices.
