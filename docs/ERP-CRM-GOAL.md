...
- [~] **E-4** Procurement & Vendors — **backend done**: `Vendor` + `PurchaseOrder` + `POLine` models + migration; `procurement` perm in both seeds + Partners role; APIs — `/api/vendors` (GET/POST), `/api/purchase-orders` (GET/POST create-with-lines computing totals), `/api/purchase-orders/[id]` (GET + PATCH submit→approve→receive state machine, approve→procurement:approve) — RBAC + tenant-scope + audit. Verified E2E (sales 403; partners create vendor + PO-0001 total 12600 → submit→approve→receive). **Next:** procurement UI (vendors + POs).
