...
- [x] **E-4** Procurement & Vendors — `Vendor` + `PurchaseOrder` + `POLine` models + migration; `procurement` perm in both seeds + Partners role; APIs (vendors GET/POST, purchase-orders create-with-lines + submit→approve→receive state machine); **UI** at `/admin/procurement` (Purchase Orders tab — list + New-PO dialog with vendor select + line items + live total + Submit/Approve/Receive buttons; Vendors tab + New-vendor dialog) + sidebar nav (EN/AR). Verified E2E (sales 403; PO-0001 total 12600 → full lifecycle) + build.
- [x] **E-5** Inventory / Assets — `Item` (unique [tenantId,sku]) + `StockMovement` models + migration; `inventory` perm in both seeds + IT-Ops role; APIs (items GET/POST + `[id]` GET/PATCH; stock-movements GET/POST that adjusts item quantity in a transaction — IN adds, OUT subtracts with **negative-stock guard**, ADJUST sets); **UI** at `/admin/inventory` (item list with live stock + New-item dialog + per-item Movement dialog) + sidebar nav (EN/AR). Verified E2E (sales 403; item IN 5→15, OUT 3→12, OUT 999→409) + build.
- [x] **E-6** Budgeting & cost centers — `CostCenter` + `Budget` models + migration; `budgeting` perm in both seeds + Finance role; APIs (cost-centers GET/POST, budgets GET/POST + PATCH record-spend) RBAC+audit; **UI** at `/admin/budgeting` (Budgets tab with animated utilization bars + record-spend; Cost Centers tab + dialogs) + sidebar nav (EN/AR). Verified E2E (cost center + 50000 budget → spend 12000 → remaining 38000) + build.
- [x] **E-7** Executive dashboards — `GET /api/erp-dashboard` (reports:view, tenant-scoped) aggregating cross-module metrics: headcount, payroll, AR, AP, inventory value, budget utilization, + counts (leads/open-opps/journal-entries/vendors); **UI** at `/admin/erp-dashboard` with framer-motion count-up KPI cards + recharts financial bar chart + budget-utilization gauge + sidebar nav (EN/AR). Verified E2E (leadership 200 with correct metrics: headcount 1, AR 11550, inventory 54000, payroll 15000, budget 24%). **Phase C ERP COMPLETE.**
...
- 2026-05-31 — E-4 Procurement backend: Vendor + PurchaseOrder + POLine models + migration (131 routes); procurement perm in both seeds + Partners role; vendors + PO (create-with-lines + submit/approve/receive state machine) APIs, RBAC+audit. Verified E2E (sales 403; PO-0001 total 12600 → full lifecycle). tsc + 44 tests + build green. Next: procurement UI, then E-5 Inventory.
- 2026-05-31 — E-4 DONE: procurement UI at /admin/procurement (POs tab with New-PO line-item dialog + live total + Submit/Approve/Receive; Vendors tab + New-vendor dialog) + sidebar nav (EN/AR). tsc + 44 tests + build (132 routes) green. Next: E-5 Inventory.
